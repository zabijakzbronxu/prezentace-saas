...
import { text, MAX, type FormState } from "@/lib/presentations/form";
 ...
 * Chyby vrací jako stav formuláře (useActionState) — hodnoty ve formuláři zůstávají.
 */
export async function updateProfile(
  ...
  if (full_name && full_name.length > MAX.contact_name) {
    return { error: `Jméno je moc dlouhé (max ${MAX.contact_name} znaků).` };
  }
  if (phone) {
    if (phone.length > MAX.contact_phone) {
      return { error: `Telefon je moc dlouhý (max ${MAX.contact_phone} znaků).` };
    }
    const digits = phone.replace(/\D/g, "");
    if (digits.length < 6 || !/^[+]?[\d\s()/.-]+$/.test(phone)) {
      return { error: "Telefon nevypadá správně — použij číslice, případně mezery a +420." };
    }
  }
